import { endsWith, last, merge, range, times } from 'lodash';
...
describe('getDiceValuesForTable', () => {
	...
	test.each(tableLengthsMappedToD100)('table of length $0 should be mapped to d100', tableLength => {
		...
		expect(results).toHaveLength(tableLength);
		assert(endsWith(last(results)!, '100'), `Last value should end with 100, but was ${last(results)}`);
	});
});

function simpleExpectedValues(size: number) {
